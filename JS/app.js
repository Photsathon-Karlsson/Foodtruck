...
async function fetchMenu() {
    try { // Using "try" to checking errors.
        const response = await fetch(`https://fdnzawlcf6.execute-api.eu-north-1.amazonaws.com/menu`, { // Send request to get info from API by using "fetch". 
...
function displayReceipt(receipt) {
  ...
  const receiptIdPage3 = document.querySelector(".receipt-code-page3"); // Set `receiptId` to reference `.receipt-code` inside `receiptContainer`.
  receiptIdPage3.textContent = receipt.order.id; // Set `receiptId` text to `receipt.order.id`.
// ( Page 4 : Receipt ) : Show reciept from API / have button to go back to menu.
  ...
  const receiptIdPage4 = receiptContainer.querySelector(".receipt-code-page4"); // Set `receiptId` to reference `.receipt-code` inside `receiptContainer`.
  ...
  receiptIdPage4.textContent = receipt.order.id; // Set `receiptId` text to `receipt.order.id`.
